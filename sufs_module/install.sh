...
# Check SUFS requirements
check_sufs_requirements() {
  log "Checking SUFS requirements..."
  
  # Check if SUFS is available
  if [ ! -d "/data/adb/sufs" ]; then
    error_exit "SUFS not detected. Please install SUFS first."
  fi
  
  # Check SUFS version
  if [ -f "/data/adb/sufs/version" ]; then
    local sufs_version=$(cat /data/adb/sufs/version)
    log "SUFS version: $sufs_version"
  else
    ui_print "WARNING: Cannot determine SUFS version"
  fi
  ...
  log "SUFS requirements check completed"
}

print_modname() {
  ui_print "********************************"
  ui_print "  Framework Patcher V2 (SUFS)"
...
EOF
  ...
  log "SUFS module installation completed successfully"
}
